...
import { getNotificationSettings } from "../modules/orders/order.controller.js";
...
async function sendOrderEmail(toEmails, notificationType, templateData) {
  ...
  if (!settings || settings.enabled === false) {
    logger.info(
      "Notification skipped because it is disabled or not configured",
      {
        notificationType,
      },
    );
  ...
  if (
    notificationType.includes("status-update") &&
    templateData?.currentStatus
  ) {
    const currentStatus = String(templateData.currentStatus)
      .toLowerCase()
      .trim();
    const allowed = Array.isArray(settings.trigger_statuses)
      ? settings.trigger_statuses
      : [];

    if (
      allowed.length > 0 &&
      !allowed.map((s) => s.toLowerCase().trim()).includes(currentStatus)
    ) {
      logger.info("Notification skipped because status is not configured", {
        notificationType,
        currentStatus,
      });
      return {
  ...
  const trackLink = escapeHtml(
    templateData.trackLink ||
      "https://royalgulfshipping.com/track-your-shipment/",
  );
  const refId = escapeHtml(templateData.refId || "—");

  let itemsHtml =
    '<p style="color:#64748b;">No specific item details available.</p>';
