...
export async function createOrder(req, res) {
  ...
  try {
    ...
    if (ownerWantsEmail) {
      ...
      if (ownerEmail) {
        if (allItemRefsList.length) {
          for (const ref of allItemRefsList) {
            emailTargets.push({
              id: senderId,
              type: "sender",
              email: ownerEmail,
    ...
    for (let i = 0; i < shippingParties.length; i++) {
      ...
      const recResult = await withUserAudit(
        ...
        [
          orderId,
          p[`${partyPrefix}_name`] || p.receiver_name || p.sender_name || "",
          p[`${partyPrefix}_contact`] ||
            p.receiver_contact ||
            p.sender_contact ||
            "",
          p[`${partyPrefix}_address`] ||
            p.receiver_address ||
            p.sender_address ||
            "",
          p[`${partyPrefix}_email`] || p.receiver_email || p.sender_email || "",
          p.receiver_marks_and_number || p.sender_marks_and_number || "",
          normEta,
          normEtd,
          "",
          "",
          ...
          receiverItemRefs,
          p[`${partyPrefix}_ref`] || p.receiver_ref || p.sender_ref || "",
        ],
      );
      ...
      if (wantsEmail) {
        if (partyEmail) {
          const partyItemRefs = items
            .map((it) => it.item_ref || it.itemRef || "")
            .filter(Boolean);

          if (partyItemRefs.length) {
            for (const ref of partyItemRefs) {
              emailTargets.push({
                id: receiverId,
                type: "receiver",
                email: partyEmail,
      ...
      for (const item of items) {
        ...
        await withUserAudit(
          ...
          `INSERT INTO order_items (
            order_id, sender_id, receiver_id, item_ref, pickup_location, delivery_address,
...
export async function getOrdersConsignments(req, res) {
  ...
  try {
    ...
    if (container_id) {
      ...
      if (consignment_id) {
        ...
        whereClause += ` AND EXISTS (
          ...
          JOIN container_assignment_history cah ON cah.detail_id = oi.id
          WHERE oi.order_id = o.id
            AND cah.cid = ANY($${chContainerParam}::int[])
            AND cah.consignment_id = $${cidParam}
        )`;
      } else {
        whereClause += ` AND EXISTS (
          SELECT 1 FROM order_items oi
          ...
          WHERE oi.order_id = o.id
            AND cah.cid = ANY($${chContainerParam}::int[])
            AND cah.consignment_id IS NULL
        )`;
    ...
    if (consignment_id && !container_id) {
      if (pol) {
      ...
      params.push(parseInt(consignment_id, 10));
      const cidParam2 = params.length;

      whereClause += ` AND EXISTS (
        SELECT 1 FROM order_items oi
        JOIN container_assignment_history cah ON cah.detail_id = oi.id
        WHERE oi.order_id = o.id
          AND cah.consignment_id = $${cidParam2}
      )`;
    }
    ...
    const shippingDetailsSub = consignment_id
      ? `
      (SELECT COALESCE(json_agg(json_build_object(
        ...
        'status', COALESCE(oi.status, 'Created'),
        'totalNumber', COALESCE(oi.total_number, 0),
        'weight', COALESCE(oi.weight, 0),
      ...
      WHERE oi.receiver_id = r.id
        AND EXISTS (
          SELECT 1 FROM container_assignment_history cah
          WHERE cah.detail_id = oi.id AND cah.consignment_id = ${parseInt(consignment_id, 10)}
        ))
    `
      : `
      (SELECT COALESCE(json_agg(json_build_object(
      ...
      `
        SELECT 
          ...
          json_agg(
            json_build_object(
              ...
              'deliveryDate',         TO_CHAR(oc.delivery_date, 'YYYY-MM-DD'),
              'createdAt',            oc.created_at,
              'status',                COALESCE(oc.status, false),
              'items', COALESCE(items.items, '[]'::json),
              'gatepass', COALESCE(gp.gatepass, '[]'::json),
              'generatedGatepasses', COALESCE(ggp.gatepasses, '[]'::json)
            ) ORDER BY oc.id DESC
          ) AS collections
        FROM order_collections oc
        LEFT JOIN LATERAL (
        ...
        LEFT JOIN LATERAL (
          SELECT json_agg(
            json_build_object(
              'id', g.id,
              'gpNumber', g.gp_number,
              'driverName', g.driver_name,
              'driverId', g.driver_id,
              'driverContact', g.driver_contact,
              'plateNo', g.plate_no,
              'pickupLocation', g.pickup_location,
              'qty', g.qty,
              'weight', g.weight,
              'commodity', g.commodity,
              'marksAndNumber', g.marks_and_number,
              'customerName', g.customer_name,
              'customerContact', g.customer_contact,
              'gateDate', TO_CHAR(g.gate_date, 'YYYY-MM-DD'),
              'createdAt', g.created_at
            ) ORDER BY g.id DESC
          ) AS gatepasses
          FROM gatepass_records g
          WHERE g.collection_id = oc.id
        ) ggp ON true
        WHERE oc.order_id = $1
        GROUP BY oc.receiver_id
       `,
...
export async function assignContainersBatch(req, res) {
  ...
  try {
    ...
    for (const ass of assignments) {
      ...
      const totalW = Number(targetItem.weight || targetItem.total_weight || 0);
      const alreadyAssignedWeight = Number(targetItem.assigned_weight_kg || 0);
      const remainingWeight = totalW - alreadyAssignedWeight;
      if (ass.assignedWeightKg != null) {
        assignWeight = Math.min(Number(ass.assignedWeightKg), remainingWeight);

        if (isNaN(assignWeight) || assignWeight < 0) {
          skipped.push({
            ...ass,
            reason: "Invalid custom assignedWeightKg",
          });
          continue;
        }
      } else {
      ...
      await client.query(
        `
          ...
          SET
            status = $1,
            assigned_boxes = $2,
            assigned_weight_kg = $3,
            container_details = $4::jsonb,
            assigned_at = NOW(),
            updated_at = NOW()
          WHERE id = $5
        `,
...
export const getAssignedOrderById = async (req, res) => {
  ...
  try {
    ...
    return res.status(200).json({
      ...
      rgl_booking_number: orderResult.rows[0].rgl_booking_number,
...
function normalizeReceiver(row) {
  ...
  return {
    ...
    receiverMarksAndNumber: row.receiver_marks_and_number,
    kycApproved,
...
export async function getPdfData(req, res) {
  let client;
  try {
    ...
    const RECEIVERS_QUERY = `
      SELECT
        r.id, r.order_id,
        r.receiver_name, r.receiver_contact, r.receiver_address, r.receiver_email, r.receiver_marks_and_number,
        r.receiver_ref,
        r.containers,
        sd_full.shippingdetails,
      ...
      LEFT JOIN LATERAL (
        SELECT json_agg(
          json_build_object(
            ...
            'category', COALESCE(oi.category, ''),
            'subcategory', COALESCE(oi.subcategory, ''),
            'itemName', COALESCE(NULLIF(oi.subcategory, ''), oi.category, ''),
            'totalNumber', COALESCE(oi.total_number, 0)::int,
            'weight', COALESCE(oi.weight, 0)::numeric,
            'itemRef', COALESCE(oi.item_ref, ''),
            ...
            'containerAssignments', COALESCE(cah_agg.assignments, '[]'::json),
            'containerDetails', COALESCE(cah_agg.legacy_shape, '[]'::json)
          ) ORDER BY oi.id
        ) AS shippingdetails
        FROM order_items oi
        LEFT JOIN LATERAL (
          SELECT eta FROM order_tracking t
          WHERE t.receiver_id = r.id AND t.item_ref = oi.item_ref
        ...
        LEFT JOIN LATERAL (
          SELECT
            json_agg(
              json_build_object(
                'container_number', cah.container_number,
                'cid', cah.cid,
                'assigned_qty', COALESCE(cah.assigned_qty, 0)::int,
                'assigned_weight_kg', COALESCE(cah.assigned_weight_kg, 0)::numeric,
                'status', cah.status,
                'action_type', cah.action_type,
                'created_at', cah.created_at
              ) ORDER BY cah.id
            ) AS assignments,
            json_agg(
              json_build_object(
                'container', json_build_object(
                  'container_number', COALESCE(cah.container_number, '')
                ),
                'total_number', COALESCE(cah.assigned_qty, 0)::int,
                'assign_weight', COALESCE(cah.assigned_weight_kg, 0)::text,
                'assign_total_box', COALESCE(cah.assigned_qty, 0)::int,
                'status', cah.status
              ) ORDER BY cah.id
            ) AS legacy_shape
          FROM container_assignment_history cah
    ...
    const DROPOFF_QUERY = `
      SELECT receiver_id, json_agg(
        json_build_object(
          'drop_method', drop_method, 'dropoff_name', dropoff_name,
          'drop_off_mobile', drop_off_mobile, 'plate_no', plate_no,
          'drop_date', TO_CHAR(drop_date, 'YYYY-MM-DD')
        ) ORDER BY id
      ) AS drop_off_details
      FROM drop_off_details WHERE order_id = $1 GROUP BY receiver_id
    `;

    const CONSIGNMENT_QUERY = `
      SELECT
        c.id,
        c.consignment_number,
        c.shipper,
        c.shipper_address,
        tp_shipper.contact_phone AS shipper_contact,
        c.consignee,
        c.consignee_address,
        tp_consignee.contact_phone AS consignee_contact,
        c.voyage,
        ...
        c.customs_crn
      FROM consignments c
      LEFT JOIN third_parties tp_shipper ON tp_shipper.id = c.shipper_id
      LEFT JOIN third_parties tp_consignee ON tp_consignee.id = c.consignee_id
      LEFT JOIN vessels v ON v.id = c.vessel
      WHERE c.orders @> to_jsonb($1::int)
      LIMIT 1
    `;
    client = await pool.connect();
  ...
  `,
    ...
    const [receiversResult, dropOffResult, consignmentResult] =
      await Promise.all([
        client.query(RECEIVERS_QUERY, [orderId]),
        client.query(DROPOFF_QUERY, [orderId]),
        client.query(CONSIGNMENT_QUERY, [orderId]),
      ]);
    ...
    const receivers = receiversResult.rows.map((row) =>
      normalizeReceiver({
        ...row,
        drop_off_details: dropOffMap.get(row.id) || [],
      }),
    );

    let consignment = null;
    if (consignmentResult.rowCount > 0) {
      const c = consignmentResult.rows[0];
      consignment = {
        id: c.id,
        ...
        shipper: {
          company_name: c.shipper || "",
          address: c.shipper_address || "",
          contact_phone: c.shipper_contact || "",
        },
        consignee: {
          company_name: c.consignee || "",
          address: c.consignee_address || "",
          contact_phone: c.consignee_contact || "",
        },
      };
    }
    ...
    console.error("Error in getPdfData:", err);
    res.status(500).json({
...
export const getOrderTracking = async (req, res) => {
  try {
    const { rows } = await pool.query(`
      ...
      LEFT JOIN container_master cm ON cm.cid = ot.container_id
      LEFT JOIN consignments con ON con.id = ot.consignment_id
      LEFT JOIN modules m ON m.id = ot.module_id
      ORDER BY ot.created_time DESC
    `);

    if (rows.length === 0) {
      logger.warn("No Tracking Data Found");
      return res.status(404).json({
        success: false,
...
export async function createOrderCollections(req, res) {
  ...
  try {
    ...
    for (let i = 0; i < collections.length; i++) {
      ...
      const createdGatepasses = [];
      if (Array.isArray(c.gatepasses) && c.gatepasses.length) {
        for (const g of c.gatepasses) {
          if (!g.driverName || !g.plateNo) continue;
          const gpNumber = `GP-${collectionId}-${Date.now().toString().slice(-6)}-${Math.floor(Math.random() * 90 + 10)}`;
          const gpRes = await client.query(
            `INSERT INTO gatepass_records
            (collection_id, gp_number, driver_name, driver_id, driver_contact,
            plate_no, pickup_location, qty, weight, commodity,
            marks_and_number, customer_name, customer_contact, gate_date)
          ...
          RETURNING *`,
            [
              ...
              g.driverName,
              g.driverId || null,
              g.driverContact || null,
              g.plateNo,
              g.pickupLocation || null,
              g.qty ? parseInt(g.qty, 10) : null,
              g.weight ? parseFloat(g.weight) : null,
              g.commodity || null,
              g.marksAndNumber || null,
              g.customerName || null,
              g.customerContact || null,
              g.gateDate || null,
            ],
          );
          createdGatepasses.push(gpRes.rows[0]);
        }
      }
...
export const listBookingFormsForImport = async (req, res) => {
  try {
    const { rows } = await pool.query(`
      SELECT
        bc.id,
        ...
        COALESCE(
          json_agg(DISTINCT jsonb_build_object(
            ...
            'signature_url', sub_s.signature_url
        ...
        COALESCE(
          json_agg(DISTINCT jsonb_build_object(
            ...
            'booking_submitted_at', r.booking_submitted_at,
            'signature_url', sub_r.signature_url
          )) FILTER (WHERE r.id IS NOT NULL), '[]'
        ) AS receivers
      FROM booking_confirmations bc
      JOIN companies c ON c.id = bc.company_id
      LEFT JOIN booking_confirmation_senders s ON s.booking_form_id = bc.id
      LEFT JOIN booking_confirmation_receivers r ON r.booking_form_id = bc.id
      LEFT JOIN booking_confirmation_submissions sub_s
        ON sub_s.form_submitter_id = s.id AND sub_s.participant_type = 'sender'
      LEFT JOIN booking_confirmation_submissions sub_r
        ON sub_r.form_submitter_id = r.id AND sub_r.participant_type = 'receiver'
      GROUP BY bc.id, c.id
      ORDER BY bc.created_at DESC
    `);
    return res.json(rows);
  } catch (err) {
    console.error("listBookingFormsForImport error:", err);
    return res.status(500).json({ error: "Failed to fetch booking forms" });
  }
};

export const getBookingFormForImport = async (req, res) => {
  ...
  try {
    ...
    if (!bcRes.rows.length) return res.status(404).json({ error: "Not found" });
    const bc = bcRes.rows[0];

    const sendersRes = await pool.query(
      `SELECT * FROM booking_confirmation_senders WHERE booking_form_id = $1`,
      [bookingFormId],
    );
    const receiversRes = await pool.query(
      `SELECT * FROM booking_confirmation_receivers WHERE booking_form_id = $1`,
      [bookingFormId],
    );

    const itemsRes = await pool.query(
      `SELECT id, category, subcategory, type, qty, weight, place_of_loading, place_of_destination
    ...
    const submitterIds = [
      ...sendersRes.rows.map((s) => s.id),
      ...receiversRes.rows.map((r) => r.id),
    ];
    let formUrl = null;
    if (submitterIds.length) {
      const urlRes = await pool.query(
        `SELECT form_url FROM booking_confirmation_submissions
         WHERE form_submitter_id = ANY($1::int[]) AND form_url IS NOT NULL
         ORDER BY submitted_at DESC LIMIT 1`,
        [submitterIds],
      );
      formUrl = urlRes.rows[0]?.form_url || null;
    }
    ...
    return res.json({
      ...
      orderData: {
        subject: bc.subject,
        message: bc.message,
        total_qty: bc.total_qty,
        total_weight: bc.total_weight,
    ...
    console.error("getBookingFormForImport error:", err);
    return res
      .status(500)
      .json({ error: "Failed to fetch booking form detail" });
  }
};
...
export async function updateCollectionStatus(req, res) {
  ...
  try {
    ...
    client = await pool.connect();
    const result = await withUserAudit(
      req,
      client,
      `UPDATE order_collections SET status = $1, updated_at = now() WHERE id = $2 RETURNING *`,
      [!!status, collectionId],
    );
...
export const createGatepass = async (req, res) => {
  ...
  try {
    ...
    if (!driverName || !plateNo) {
      return res
        .status(400)
        .json({ error: "Driver name and truck number are required" });
    }

    client = await pool.connect();

    const collectionRes = await client.query(
      "SELECT id FROM order_collections WHERE id = $1",
      [collectionId],
    );
    ...
    const gpNumber = `GP-${collectionId}-${Date.now().toString().slice(-6)}`;
    ...
    return res.status(201).json({ success: true, gatepass: result.rows[0] });
  } catch (error) {
...
export const getGatepassesForCollection = async (req, res) => {
  let client;
  try {
    const { collectionId } = req.params;
    client = await pool.connect();
    const result = await client.query(
      "SELECT * FROM gatepass_records WHERE collection_id = $1 ORDER BY id DESC",
      [collectionId],
    );
    return res.json({ gatepasses: result.rows });
  } catch (error) {
